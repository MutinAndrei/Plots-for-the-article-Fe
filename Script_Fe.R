...
p1 <- ggplot(mart23, aes(x = xЛВ2, y = yFeCl2, col = "gray65")) + geom_point(size = 2, col = "gray27") + 
  ...
  ylab('L-lactate conc. by Fe, mM') + 
  xlab('L-lactate conc. by LV, mM') +
  ggtitle("NaCl (5 g/L)") +
...
p5 <- ggplot(mart23, aes(x = x, y = conc)) + geom_point(size = 2, col = "gray27") +
  ...
  ggtitle("Calibration curve for Fe") +
  xlim(0, 15) +
  ylim(0, 2) +
  theme(plot.margin = unit(c(0.5,0.5,0.5,0.5), 'lines')) +
  theme(plot.title = element_text(hjust = 0.02)) +
  ...
  annotate("text", x = c(3.5), y = c(1.8),          #Linear regression formula
           label = 'atop("y = 0.143x + 0.1")', cex=5, parse = TRUE, family = "Arial") +
...
p6 <- ggplot(mart23, aes(x = NomCONC, y = RealCONC)) + geom_point(size = 2, col = "gray27") +
  ...
  theme(text = element_text(size = 15, family = "Arial", colour = "black"),
        axis.text=element_text(color="black", size = 15)) + 
  ylab('Measured conc., mM') + 
  xlab('Nominal conc., mM') +
  ggtitle("Concentration of lactic acid solutions") +
  ...
  annotate("text", x = c(3.5), y = c(13.2),          #Linear regression formula  
           label = 'atop("y = 1.03x - 0.392")', cex=5, parse = TRUE, family = "Arial") +
  annotate("text", x = c(3.5), y = c(12.7), 
           label = "R² = 0.99", cex=5, family = "Arial") +
  theme(panel.background = element_rect(fill = 'gray97', colour = 'gray97'))
...
ggarrange(p6,
          ...
          nrow = 3,
          ncol = 2,
          labels = c("A","B","C","D","E","F"),font.label = list(size = 17))

#Save Fig.1
ggsave("Fig1.png", width = 24, height = 26, units = "cm", dpi = 300)
...
#Creating a graph for a panel from a single graph (Fig2 v.1)
ggplot(f3, aes(x = nm, y = T, group = Group)) + 
  ...
  scale_linetype_manual(values = tipline3, 
                        breaks = c("FeCl11mM",
                                   "PVK01mM", "lactateD01mM", 
                                   "PVK05mM", "lactateD05mM", 
                                   "lactateD1mM", "PVK1mM",
                                   "PVK2_5mM", "lactateD2_5mM", 
                                   "PVK5.0mM", "lactateD5mM", 
                                   "PVK10mM", "lactateD10mM"),
                        labels = c("11 мМ FeCl₃",
                                   "0.1 mM PA", "0.1 mM LA", 
                                   "0.5 mM PA", "0.5 mM LA",
                                   "1 mM PA", "1 mM LA",
  ...
  xlab("Wavelength, nm") + 
  theme(panel.background = element_rect(fill = 'gray97', colour = 'gray97')) +
  theme(text = element_text(size = 14, family = "Arial", colour = "black"),
        axis.text=element_text(color="black", size = 13)) 

#Save Fig2 v.1
ggsave("spectrsf3.JPG", width = 17, height = 10, units = "cm", dpi = 300)



#Creating a graph for a panel of two graphs (Fig2 v.2)
pf3 <- ggplot(f1, aes(x = nm, y = T, group = Group)) + 
  geom_line(aes(color = Group, linetype = Group)) +
  labs(color = '', linetype = '') +
  scale_linetype_manual(values = tipline1, 
                        breaks = c("FeCl11mM", 
                                   "PVK01mM", "lactateD01mM", 
                                   "PVK05mM", "lactateD05mM", 
                                   "lactateD1mM", "PVK1mM"),
                        labels = c("11 мМ FeCl₃", 
                                   ...
                                   "0.5 mM PA", "0.5 mM LA",
                                   "1 mM PA", "1 mM LA")) +
  scale_color_manual(values = cvet1,
                     breaks = c("FeCl11mM", 
                                "PVK01mM", "lactateD01mM", 
                                "PVK05mM", "lactateD05mM", 
                                "lactateD1mM", "PVK1mM"),
...
#Creating a graph for a panel of two graphs (Fig2 v.2)
pf2 <- ggplot(f2, aes(x = nm, y = T, group = Group)) + 
  geom_line(aes(color = Group, linetype = Group)) +
  labs(color = '', linetype = '') +
  scale_linetype_manual(values = tipline2, 
                        breaks = c(
                          "PVK2_5mM", "lactateD2_5mM", 
                          "PVK5.0mM", "lactateD5mM", 
                          "PVK10mM", "lactateD10mM"),
                        labels = c( 
                          ...
                          "5 mM PA", "5 mM LA", 
                          "10 mM PA", "10 mM LA")) +
  scale_color_manual(values = cvet2,
                     breaks = c(
                       "PVK2_5mM", "lactateD2_5mM", 
                       "PVK5.0mM", "lactateD5mM", 
                       "PVK10mM", "lactateD10mM"),
...
#Creating a panel of two graphs (Fig2 v.2)
ggarrange(pf3,
          pf2,
          nrow = 1,
          ncol = 2,
...
#Save Fig2 v.2
ggsave("spectrsfmixf1andf2.JPG", width = 23, height = 10, units = "cm", dpi = 300)
